...
function stableStringify(value: unknown): string {
  ...
  if (value && typeof value === "object") return `{${Object.keys(value as object).sort().map((key) => `${JSON.stringify(key)}:${stableStringify((value as UnknownRecord)[key])}`).join(",")}}`;
...
function sha256(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const older = words[index - 2] ?? 0;
      const smallSigma0 = rotateRight(previous, 7) ^ rotateRight(previous, 18) ^ (previous >>> 3);
      const smallSigma1 = rotateRight(older, 17) ^ rotateRight(older, 19) ^ (older >>> 10);
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + bigSigma1 + choose + (ROUND_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0;
